...
function getNextFriday(date) {
  ...
  const day = ddate.getUTCDay();
  const daysToFriday = (5 - day + 7) % 7 || 7;
  ddate.setUTCDate(ddate.getUTCDate() + daysToFriday);
  return ddate;
}
...
function isDateInPeriod(date, period) {
  const startInc = new Date(period.start);
  const endInc = new Date(period.end);
  const ddate = new Date(date);
  return startInc <= ddate && ddate <= endInc;
}
...
function formatDate(date) {
  ...
  const timeString = sdate.toLocaleTimeString('en-US', { timeZone: 'UTC' });
...
function getWeekNumberByDate(date) {
  const yearBeginning = new Date(date.getFullYear(), 0, 0);
  return Math.ceil((date - yearBeginning) / 1000 / 60 / 60 / 24 / 7);
}
...
function getQuarter(date) {
  const monNum = date.getMonth() + 1;
  let result = 0;
  if (monNum < 4) {
    result = 1;
  }
  if (monNum > 3 && monNum < 7) {
    result = 2;
  }
  if (monNum > 6 && monNum < 10) {
    result = 3;
  }
  if (monNum > 9 && monNum < 13) {
    result = 4;
  }
  return result;
}
